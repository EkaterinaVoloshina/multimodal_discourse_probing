...
from volta.task_utils import EvaluatingModel, LoadDatasetEval, LoadLoss
...
def evaluate(model, valid_dataset, log_file, device):
    ...
    for batch in tqdm(valid_dataset):
        ...
        seq_relationship_score = torch.nn.functional.softmax(
            seq_relationship_score, dim=-1
        )
        ...
        accuracy.append(
            sum(np.argmax(seq_relationship_score, axis=1) == is_match) / batch_size
        )

        log = [
            {
                "id": ids[i],
                "is_match": float(is_match[i]),
                "score": seq_relationship_score[i].tolist(),
            }
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a model in zero-shot format."
    )
