...
def load_dataset(filename, dirname):
    ...
    for num, image in tqdm(enumerate(images)):
        ...
        if f"{idx}.jpg" not in os.listdir(dirname):
            if "url_o" in image:
                img = open_and_save(image["url_o"], filename=image["id"], dirname=dirname)
            elif "url_m" in image:
                img = open_and_save(image["url_o"], filename=image["id"], dirname=dirname)
            else:
                pass
...
def main():
    ...
    parser.add_argument("-d", "--dirname", type=str, help="The directory with json files",
                        default="dii")
    parser.add_argument("-o", "--output_dir", type=str, help="The output directory with images",
                        default="vist_images")
