...
class DatasetLoader(Dataset):
    def __init__(self, dir_name="images",
                 image_size=256):
    ...
    def load_dataset(self, dir_name):
        image_ids = []
        image_tensor = torch.Tensor(len(os.listdir(dir_name)), 3, self.image_size, self.image_size)
...
def main():
    parser = argparse.ArgumentParser(description="Calculate the image similarity in the dataset")
    parser.add_argument("--dirname", default="images", help="the directory where the images are saved")
    parser.add_argument("--output_dir", default="vectors", help="the output directory for the vectors")
    parser.add_argument("--batch_size", default=8, help="the batch size of the encoder model")
    parser.add_argument("--image_size", default=256, help="the size of images to encode")
