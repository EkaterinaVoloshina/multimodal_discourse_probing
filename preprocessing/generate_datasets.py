...
def confuse_captions(vectors, images):
    ...
    for i, vector in enumerate(cos_sim):
        ...
        similar_vectors[images[i]] = list(
            zip(map(lambda x: images[x], similar), map(str, similar_values))
        )
        dissimilar_vectors[images[i]] = list(
            zip(map(lambda x: images[x], dissimilar), map(str, dissimilar_values))
        )
...
def generate_datasets(
    captions, text_vectors, text_ids, image_vectors, image_ids, output_dir
):
    ...
    write_json(
        os.path.join(output_dir, "caption_valid_similar_captions.json"),
        similar_captions,
    )

    dissimilar_captions = get_caption_by_similarity(captions, dissimilar_captions)
    write_json(
        os.path.join(output_dir, "caption_valid_dissimilar_captions.json"),
        dissimilar_captions,
    )

    similar_images = get_caption_by_similarity(
        captions, similar_images, shuffle_captions=False
    )
    write_json(
        os.path.join(output_dir, "caption_valid_similar_images.json"), similar_images
    )

    dissimilar_images = get_caption_by_similarity(
        captions, dissimilar_images, shuffle_captions=False
    )
    write_json(
        os.path.join(output_dir, "caption_valid_dissimilar_images.json"),
        dissimilar_images,
    )
...
def main():
    ...
    parser.add_argument(
        "--dirname", default="vectors", help="a directory where vectors are saved."
    )
    parser.add_argument(
        "--output_dir", default="datasets", help="a directory to save datasets to."
    )
    ...
    image_vectors, image_ids = load_data(
        args.dirname, "image_vectors.npy", "image_ids.txt"
    )

    print("Generating data..")
    generate_datasets(
        captions, text_vectors, text_ids, image_vectors, image_ids, args.output_dir
    )
    print("Done!")
